...
class VideoPlayer(tk.Toplevel):
    def __init__(self, parent, file_name):
        ...
        self.media = self.instance.media_new(
            f"http://127.0.0.1:8000/media/videos/{file_name}"
        )
        ...
        self.pause_button = tk.Button(
            self.control_frame, text="Pause", command=self.pause
        )
...
class FlightPlanDesignerWindow:
    ...
    def saveToDB(self, win, title):
        ...
        response = requests.post(
            "http://127.0.0.1:8000/add_waypoints", json=data, headers=headers
        )
        # print(response.json())
    ...
    def populate_table(self):
        ...
        for flight in flights:
            ...
            self.table.insert(
                "",
                "end",
                values=(formatted_date, flight["NumPics"], flight["NumVids"]),
    ...
    def displayPreviousFlights(self):
        ...
        self.table = Treeview(
            self.pfFrame, columns=("Date", "NumPics", "NumVids"), show="headings"
        )
        self.table.grid(row=1, column=0, padx=20)
    ...
    def drawFlighPlan(self, waypoints):
            ...
            if self.mode == 4 or self.mode == 0:
                if wp["staticVid"]:
                    self.canvas.itemconfig(
                        self.waypointsIds[wpNum - 1]["ovalId"], fill="green"
                    )
                elif self.mode == 4 and wp["takePic"]:
                    self.canvas.itemconfig(
                        self.waypointsIds[wpNum - 1]["ovalId"], fill="red"
                    )
                elif self.mode == 0 and wp["takePic"]:
                    self.canvas.itemconfig(
                        self.waypointsIds[wpNum - 1]["ovalId"], fill="orange"
                    )
                if vidStarted:
                    if wp["endVid"]:
                        vidStarted = False
                    ...
                    self.canvas.itemconfig(
                        self.waypointsIds[wpNum - 1]["lineInId"], fill="green"
                    )
                if wp["startVid"]:
                    vidStarted = True
                    self.canvas.itemconfig(
                        self.waypointsIds[wpNum - 1]["lineOutId"], fill="green"
                    )
                    if self.mode == 0:
                        self.canvas.itemconfig(
                            self.waypointsIds[wpNum - 1]["ovalId"], fill="red"
        ...
        lineId = self.canvas.create_line(
            posx, posy, self.originx, self.originy, fill="blue"
        )
        g = self.geod.Inverse(
            float(prev["lat"]),
            float(prev["lon"]),
    ...
    def loadFromDB(self, win):
        win.destroy()
        flightPlans = requests.get("http://127.0.0.1:8000/get_all_flightPlans").json()[
            "Waypoints"
        ]
    ...
    def openImagesWindow(self):
        ...
        for i, img_file in enumerate(
            ...
            img_label.bind(
                "<Button-1>", lambda e, file=img_file: self.display_image(file)
            )
                            ...
                            if item_color == "green":
                                ...
                                static_video_path = next(
                                    (
                                        video["path"]
                                        for video in self.currentFlightData["Videos"]
                                        if video["mode"] == "static"
                                        and video["lat"] == matching_waypoint["lat"]
                                        and video["lon"] == matching_waypoint["lon"]
                                    ),
                                    None,
                                )
                                ...
                                picture_path = next(
                                    (
                                        picture["path"]
                                        for picture in self.currentFlightData[
                                            "Pictures"
                                        ]
                                        if picture["lat"] == matching_waypoint["lat"]
                                        and picture["lon"] == matching_waypoint["lon"]
                                    ),
                                    None,
                                )
                                if picture_path:
    ...
    def drawShortestPath(self, waypoints, length, lengthOriginal):
        ...
        if choice_window.result == "No":
            ...
            self.drawFlighPlan(waypoints[:-1])
